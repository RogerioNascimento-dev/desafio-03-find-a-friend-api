...
describe('Organization Controller', async () => {
  ...
  it('Should be able create organization.', async () => {
    ...
    const response = await request(kernel.server).post('/organizations').send(orgMock)    
  ...
  it('Should not be able create organization with duplicated email.', async () => {
    ...
    const response = await request(kernel.server).post('/organizations').send(orgMock)
    expect(response.statusCode).toBe(400)
    expect(response.body.message).toBe('Already exists an organization with this email.')    
  })
})